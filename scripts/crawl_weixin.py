...
class Spider:
    ...
    def gen_item_map(self, item_list):
    ...
    def html_content_strip(self, html_str):
        ...
        raw_content = self.gen_item_map(item_map)
        return raw_content
...
class WeiXin(Spider):
    ...
    def crawl(self):
        ...
        content = requests.get(self.url, headers=headers, params=data).text
        self.html_content = content
        # open("../data/weixin_html.json", "w", encoding='utf-8').write(content)

    def get_content(self):
        self.crawl()
        content_obj = json.loads(self.html_content)
        title = content_obj["title"]
        desc = content_obj["desc"]
        html_content = content_obj["content_noencode"]
        plain_content = self.html_content_strip(html_content)
        return title + "\n" + desc + "\n" + plain_content
...
if __name__ == '__main__':
    ...
    print(general_utils.isURL(url))
    # wx = WeiXin(url)
    # data = wx.get_content()
    # print(data)
    # crawl_weixin(url)
    # for line in open("../data/weixin_html.json", 'r', encoding='utf-8').readlines():
